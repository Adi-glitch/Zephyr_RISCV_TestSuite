 ...
 *
 * Copy of the suite's common/smode.c. The ONLY change vs the original is the
 * pass/fail verdict sink: the original writes the HTIF `tohost` word (read by
 * Spike's testbench); here we write the QEMU virt `sifive_test` finisher so the
 * emulator exits with a pass/fail code. Test logic is untouched.
 */
...
/* --- Verdict: QEMU virt "sifive_test" finisher (replaces HTIF tohost) --- */

#define QEMU_SIFIVE_TEST 0x00100000UL
#define QEMU_FINISH_PASS 0x5555U
#define QEMU_FINISH_FAIL 0x3333U

static void qemu_test_exit(uint32_t value)
{
    *(volatile uint32_t *)QEMU_SIFIVE_TEST = value;
    while (1)
        asm volatile("wfi");
}
...
{
    uart_puts("[PASS]\n");
    qemu_test_exit(QEMU_FINISH_PASS);
}
...
void test_fail(int code)
{
    ...
    uart_put_dec(code);
    uart_putc('\n');
    /* QEMU exits with `code` as the process exit status. */
    qemu_test_exit(QEMU_FINISH_FAIL | ((uint32_t)code << 16));
}
